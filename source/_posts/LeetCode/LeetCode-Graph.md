...
3  十字链表:
4  [前向星](http://blog.csdn.net/qq_30331643/article/details/68621435):  向前星是一个数组记录以某个点为起点的边，里面的element 是 Edge { start, end }, 按照先start,后end 拍好序，
再用 len[i]来记录所有以i为起点的边在数组中的存储长度,head[i]记录以i为边集在数组中的第一个存储位置.
 所以出度为0的边，是不能存进数组的。
